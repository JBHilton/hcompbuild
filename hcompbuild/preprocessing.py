...
from numpy import (
        append, arange, array, concatenate, cumsum, diag, hstack,
        identity, insert, kron, ndarray, ones, ones_like, prod,
...
def make_initial_condition_by_eigenvector(growth_rate,
                                         ...
                                         R_comp = 4,
                                         S_comp = 0):
    '''Makes state distribution vector for initial conditions corresponding to
    the exponential growth phase at specified levels of infectious prevalence
    ...
    def __init__(self,
        ...
        if {'fine_bds', 'coarse_bds', 'pop_pyramid_file_name'} <= spec.keys():
            ...
            if self.no_age_classes==1:
                ...
                if spec['k_home']['file_name'][-4:] == '.csv': # Use read_csv if possible, otherwise default to read_excel
                    self.k_home = read_csv(
...
def map_SEPIR_to_SEPIRQ(SEPIR_population, SEPIRQ_population):
    '''Constructs a matrix which maps the state (S,E,P,I,R)
...
def AR_by_size(household_population, H, R_comp, S_comp=0):
    '''Calculates hh size-stratified attack ratio from
    simulation results.'''
